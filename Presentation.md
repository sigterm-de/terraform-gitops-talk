...
- [x] tf - kein syntax
- [] Code - korrekte referenzen 
- [] 2.2.2 weglassen 
- [] 2.4.3 mehr worte 
- [] central repo wo/ variables/parameter etc. pp
...
Each of those solves some issues and creates new ones

---
## (1.2) The boring-registry 

[.column]
- At TIER we developed our own "boring" Terraform registry without moving parts (hence the name)
...
### some Architectural Decision Records
...
- Layer your stacks - base and services
...
module "ssm_service_data" {
  ...
  data = {
    ...
    public = {
      sns = {
        "foo" = module.sns_foo.arn
      }
      sqs = {
        "bar" = module.bar_queue.arn
      }
    }
  }
}
...
module "sns_sqs_subscription_foo" {
  ...
  sns_arn        = nonsensitive(module.ssm_data.values["upstream"]["public"]["sns"]["foo"])
...
- in Flux repo there are **NO** variables, parameters etc. pp.
  - we only document **the intent** to run a service
  - self-configuration inside of an environment
...
---
### (2.2.2) A FluxCD application

[.column]
[.code-highlight: 1-2, 8, 13-24]
```yaml
...
spec:
  interval: 30m
  path: flux-apps/service-stacks/foo
  prune: true
  sourceRef:
    kind: GitRepository
    name: fluxcd
...
spec:
  ...
  provider: aws
  ref:
    semver: "${version_policy_agent_tf}"
...
[.column]
- release process
  - `_versions.yaml` -> `service-versions` ConfigMap
- basic information for a TF stack
  - `base` ConfigMap provides `client`, `environment`
  - to form the path for Terraform state file

[.column]
[.code-highlight: 6-12, 18-23]
```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: service-versions-system
data:
  version_policy_agent: "2.5.0"
  version_policy_agent_tf: "~ 0.1.0-0"
  version_policy_agent_tf_policies: "~ 0.0.1-0"
  version_vertical_pod_autoscaler: "~> 9.0"
  version_vertical_pod_autoscaler_tf: "~ 0.1.0-0"
  version_weaveworks_gitops: "~> 4.0.27"
---
apiVersion: v1
kind: ConfigMap
metadata:
...
---
## (2.4.1) Configuration Management ADRs
...
- Each **AWS/Kubernetes environment** allows a stack to become concious
  - `base` SSM data and Cluster `init` ConfigMap
  - pull of configiguration vs. push
- per **stack** - kustomize style data baked into artifact
    - terraform - `configuration.tf`
    - kustomize - `/overlays/tenant/environment/`
    - helm - `/values/tenant/environment/values.yaml`
...
[.column]
- **ADR**: no persistent pods between runs
- `terraform-provider-aws_5.31.0_darwin_arm64.zip` = 84MB
- NAT costs (AWS issue)
  - reconcile every 30'
...
- use S3 VPC endpoints :wink:

---
### (2.5.2) K8s resources

[.column]
- each reconcile cycle triggers tf-runner pods
...
[.column]
[.code-highlight: 3, 5, 10, 15, 19, 21]
```yaml
...
```
---
## (2.6) Weave Policy Engine

[.column]
- auto approve Terraform changes
  - **ADR**: no destroy/recreate
  - **ADR**: no direct IAM resources (only via controlled modules)
- :warning: **not an easy task - talk of its own** 
- https://docs.gitops.weave.works/docs/policy/intro/
- https://weaveworks.github.io/tf-controller/use-tf-controller/flux-receiver-and-alert/ 
...
- slow development of tf-controller, thank you HashiCorp
  ...
  - the talk uses features from a pre-release branch[^14]
...
- Branch planner needs to be implemented to enable developer ownership
...
- development and local testing of TF code is hard
  - possibly via [Branch Planner](https://weaveworks.github.io/tf-controller/branch-planner/)
  - only for Github
- Terraform module registry - so batteries included for devs?
  yes, kind of, but
  - Terraform understanding needed
  - it is hard to grock the stack data exchange concept
  - we provide template repos, use case documentation
...
- Secrets managed via External-Secret Operator
...
---
# (5.0) Tech stack used at Lynqtech
...
[^12]: https://fluxcd.io/flux/cmd/flux_push_artifact/
